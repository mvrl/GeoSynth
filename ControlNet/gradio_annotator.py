...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            low_threshold = gr.Slider(
                label="low_threshold", minimum=1, maximum=255, value=100, step=1
            )
            high_threshold = gr.Slider(
                label="high_threshold", minimum=1, maximum=255, value=200, step=1
            )
            resolution = gr.Slider(
                label="resolution", minimum=256, maximum=1024, value=512, step=64
            )
    ...
    with gr.Row():
        with gr.Column():
            ...
            value_threshold = gr.Slider(
                label="value_threshold", minimum=0.01, maximum=2.0, value=0.1, step=0.01
            )
            ...
            resolution = gr.Slider(
                label="resolution", minimum=256, maximum=1024, value=384, step=64
            )
            run_button = gr.Button(label="Run")
        with gr.Column():
            gallery = gr.Gallery(label="Generated images", show_label=False).style(
    ...
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(source="upload", type="numpy")
            alpha = gr.Slider(
                label="alpha", minimum=0.1, maximum=20.0, value=6.2, step=0.01
            )
            resolution = gr.Slider(
                label="resolution", minimum=256, maximum=1024, value=384, step=64
            )
    ...
    run_button.click(
        fn=midas, inputs=[input_image, resolution, alpha], outputs=[gallery]
    )
    ...
    run_button.click(
        fn=openpose, inputs=[input_image, resolution, hand], outputs=[gallery]
    )
